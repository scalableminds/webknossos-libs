...
docker run -v <host path>:/data --rm scalableminds/webknossos-cuber:wkw wkcuber.cubing --layer_name color /data/source/color /data/target
docker run -v <host path>:/data --rm scalableminds/webknossos-cuber:wkw wkcuber.cubing --layer_name segmentation /data/source/segmentation /data/target

# Convert Knossos cubes to wkw cubes
docker run -v <host path>:/data --rm scalableminds/webknossos-cuber:wkw wkcuber.convert --layer_name color /data/source /data/target
...
docker run -v <host path>:/data --rm scalableminds/webknossos-cuber:wkw wkcuber.compress --layer_name segmentation /data/target /data/target_compress

# Create metadata
docker run -v <host path>:/data --rm scalableminds/webknossos-cuber:wkw wkcuber.metadata --name great_dataset --scale 11.24,11.24,25 /data/target
```
