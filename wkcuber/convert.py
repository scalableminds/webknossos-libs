...
from .utils import add_jobs_flag, add_verbose_flag, open_wkw, open_knossos, WkwDatasetInfo, KnossosDatasetInfo, ParallelExecutor
...
if __name__ == '__main__':
    ...
    source_knossos_info = KnossosDatasetInfo(args.source_path, args.layer_name, args.dtype, args.mag)
    target_wkw_info = WkwDatasetInfo(args.target_path, args.layer_name, args.dtype, args.mag)

    with open_knossos(source_knossos_info) as source_knossos, \
         ParallelExecutor(args.jobs) as pool:
        for cube_xyz in source_knossos.list_cubes():
            pool.submit(convert_cube_job, cube_xyz, source_knossos_info, target_wkw_info)
