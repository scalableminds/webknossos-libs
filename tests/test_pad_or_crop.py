...
def test_pad_or_crop_to_size_and_topleft():
    ...
    for shape, expected_voxel_count in zip(test_shapes, expected_voxel_counts):
        ...
        cube_data_from_top_left = cube_data[:, target_topleft[1]:, target_topleft[2]:, :]
        cube_data_from_top_left_to_size = cube_data_from_top_left[:, :target_size[1], :target_size[2], :]
