...
def test_to_target_datatype_segmentation_layer():
    data = np.array(
        [
            [
                [[0, 0, 1, 5, 27], [3200, 3, 2, 1, 5], [0, 3, 2, 1, 5]],
                [[3200, 3, 2, 1, 5], [0, 0, 1, 5, 27], [6, 6, 6, 6, 6]],
            ]
        ]
    ).astype("uint32")
    ...
    converted_segmentation = to_target_datatype(
        data, target_dtype, is_segmentation_layer=True
    )
    converted_no_segmentation = to_target_datatype(
        data, target_dtype, is_segmentation_layer=False
    )
