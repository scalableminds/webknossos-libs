...
- [`webknossos convert`](convert.md#convert): Convert an image stack (e.g., `tiff`, `jpg`, `png`, `bmp`, `dm3`, `dm4`) to a WEBKNOSSOS dataset, optionally uploading directly to a server with `--upload`
...
- [`webknossos downsample`](downsample.md): Downsample a WEBKNOSSOS dataset (local or on a WEBKNOSSOS server)
...
- [`webknossos upsample`](upsample.md): Upsample a WEBKNOSSOS dataset (local or on a WEBKNOSSOS server)
