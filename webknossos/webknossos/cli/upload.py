...
def main(
    *,
    ...
    transfer_mode: Annotated[
        TransferMode,
        typer.Option(
            help="The transfer mode to use. 'http' is the default. Other modes like 'copy', 'move+symlink', 'symlink' are for users with direct filesystem access to the WEBKNOSSOS datastore."
        ),
    ] = TransferMode.HTTP,
) -> None:
