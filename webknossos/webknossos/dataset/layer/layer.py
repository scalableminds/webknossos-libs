...
class Layer(AbstractLayer):
    ...
    def transform(
        ...
        *,
        ...
        """Transforms this layer's data into another layer using an arbitrary coordinate transform.
        ...
        Args:
            output_layer: Existing layer to write the transformed data to. Must have the same
                dtype and number of channels as this layer. May belong to a different dataset.
            ...
            mag: Magnification to transform. Defaults to the finest available mag of this layer.
        ...
        Raises:
            KeyError: If the mag does not exist in this layer or in input_mask_layer.
        ...
        if self.dtype != output_layer.dtype:
            raise ValueError(
                ...
                + f"does not match this layer's dtype ({self.dtype})."
            )
        if self.num_channels != output_layer.num_channels:
            raise ValueError(
                f"The number of channels of the output layer ({output_layer.num_channels}) "
                + f"does not match this layer's number of channels ({self.num_channels})."
            )

        if mag is None:
            mag = self.get_finest_mag().mag
        else:
            mag = Mag(mag)
            if mag not in self.mags:
                raise KeyError(
                    f"Failed to transform layer {self.name}: mag {mag} does not exist."
                )
        ...
        if not isinstance(output_bbox, BoundingBox):
            raise ValueError(
                "Layer.transform is only supported for 3D (x, y, z) output bounding boxes, "
        ...
        if output_layer.bounding_box.is_empty():
            ...
            output_layer.bounding_box = output_layer.bounding_box.extended_by(
                output_bbox
            )
        ...
        if progress_desc is None:
            progress_desc = (
                f"Transforming layer {self.name} into layer {output_layer.name}"
            )
        with wrap_executor(executor) as actual_executor:
    ...
    def transform_affine(
        ...
        *,
        ...
        """Transforms this layer's data into another layer using an affine transformation.
        ...
        Args:
            ...
            mag: Magnification to transform. Defaults to the finest available mag of this layer.
            output_bbox: Region of the output layer to fill, in Mag(1) coordinates. Defaults to
                the bounding box of this layer's bounding box transformed with the affine matrix.
            translate_to_positive: If True, an output bounding box with negative topleft is
        ...
        Examples:
            ...
            output_layer = output_dataset.add_layer_like(layer, layer.name)
            layer.transform_affine(output_layer, scale_by_2)
            ```
        """
        ...
        if output_bbox is None:
            if not isinstance(self.bounding_box, BoundingBox):
                raise ValueError(
                    "Layer.transform_affine is only supported for layers with 3D (x, y, z) "
                    + f"bounding boxes, got {type(self.bounding_box).__name__} for layer {self.name}."
                )
            output_bbox = forward_transform.transform_bbox(self.bounding_box)
