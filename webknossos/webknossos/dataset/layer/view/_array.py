...
from webknossos.dataset._utils.tinywkw import TinyWkwDataset
...
class WKWArray(BaseArray):
    ...
    _cached_wkw_dataset: wkw.Dataset | TinyWkwDataset | None
    ...
    @property
    def _wkw_dataset(self) -> wkw.Dataset | TinyWkwDataset:
        if self._cached_wkw_dataset is None:
            if is_fs_path(self._path):
                ...
                try:
                    self._cached_wkw_dataset = TinyWkwDataset.open(self._path)
                except Exception as e:
                    raise ArrayException(
                        f"Exception while opening WKW array for {self._path}"
                    ) from e
