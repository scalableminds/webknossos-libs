...
- Added catchable exceptions for the ways image conversion can fail because of its input, so that callers of `Dataset.from_images`/`Dataset.add_layer_from_images` no longer have to match on error messages. All of them subclass the new `webknossos.ImageConversionError`, which subclasses `ValueError` (what these call sites raised before), and carry the offending `path`. [#1496](https://github.com/scalableminds/webknossos-libs/pull/1496)
    - `UnsupportedImageFormatError`: no reader handles this format. Also carries the `suffix`, the `supported_suffixes`, and `missing_extras` — the extras to install when the format is only unsupported because an optional dependency is missing, e.g. `.ims` without `webknossos[ims]`.
...
- Added support for converting plain Zarr arrays (v2 and v3), OME-Zarr multiscale groups (NGFF 0.4 and 0.5), N5 datasets and Neuroglancer precomputed volumes via `Dataset.from_images`/`add_layer_from_images`, built on `tensorstore` so these also work with remote (`s3://`/`gs://`/`http(s)://`) input paths. A Zarr/N5 store is recognized whether or not its directory is named `*.zarr`/`*.n5`. For a multiscale source (an OME-Zarr group, an N5 `s0`/`s1`/... pyramid, or a neuroglancer precomputed volume, which is inherently multiscale), the finest resolution is picked automatically and written as `Mag(1)`; convert the coarser levels afterwards with `Layer.downsample()`/`Dataset.downsample()`, as with every other converted layer. [#1504](https://github.com/scalableminds/webknossos-libs/pull/1504)
...
- Several channels are now only written into a single layer when they are actually colour: three uint8 channels of an image format that stores colour (`.png`, `.jpg`, `.bmp`, …), which WEBKNOSSOS displays as RGB. The channels of every other source — `.ims`, MRC, TIFF, CZI, DM3/DM4 — are separate acquisitions and become one layer each, whatever their number and dtype. `Dataset.from_images` splits them; `Dataset.add_layer_from_images` raises `UnsupportedImageDataError` unless `allow_multiple_layers=True` (write one layer per channel) or `channel=<index>` (convert one channel) is given, since it adds a single layer by contract. Previously the dtype alone decided, so three 16-bit channels were written into one layer that WEBKNOSSOS cannot display, while an RGB `.png` was split into three grayscale layers by `from_images`. [#1496](https://github.com/scalableminds/webknossos-libs/pull/1496)
...
- `Dataset.add_layer_from_images` now converts a directory of RGB images into a single RGB layer, instead of raising `UnsupportedImageDataError` about their three channels. Whether channels count as colour was decided from the suffix of the path it was given, and a directory has none. The same images passed as a glob pattern or as a list of files already converted, as did `Dataset.from_images`, which passes lists. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
- Converting a colour image (`.png`, `.jpg`, …) no longer emits a spurious "Some images are larger than expected" warning. The expected bounding box reported the channel count as the x extent, so it never matched what was written. The written data and the layer's final bounding box were always correct. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
- Fixed the axis order guessed for very small images: a grayscale image three pixels wide was read as a one-pixel RGB image, and a single-row colour image as three channels of a one-pixel image. The guess could not tell a channel axis from a real extent of length 1 or 3; every reader now reports its own axes instead. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
