...
### Fixed
- Sped up `Dataset.from_images`/`add_layer_from_images` for the chunk-based formats (Zarr, OME-Zarr, `.ozx`, N5, Neuroglancer precomputed, `.ims`, MRC, `.czi`). Every chunk was materialized into Fortran order before being written, which for a large shard is a full cache-hostile transposing copy — measurably the most expensive step of the conversion. The memory layout is now left to the storage backends, which arrange each chunk the way their format needs anyway. [#1528](https://github.com/scalableminds/webknossos-libs/pull/1528)
