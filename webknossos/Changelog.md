...
- The private `Vector3` aliases of `webknossos.annotation.annotation`, `webknossos.annotation.volume_layer`, `webknossos.skeleton.group`, `webknossos.skeleton.skeleton` and `webknossos.skeleton.tree` were replaced by the shared `Vec3Float`. They were never exported from the `webknossos` module and are therefore not covered by the [stability policy](./stability_policy.md), but `from webknossos.annotation import Vector3` used to work and now raises an `ImportError`. [#1491](https://github.com/scalableminds/webknossos-libs/pull/1491)
- `Vec3Float`, `Vec3Int` and `VecInt` declare `__slots__`, which takes 40 bytes off every instance, e.g. 4 MB for a skeleton of 100k nodes that carry a rotation. As a consequence their instances can no longer be the target of a `weakref`. [#1491](https://github.com/scalableminds/webknossos-libs/pull/1491)
- Constructing a `Vec3Int` from three ints or from a tuple of three ints is about six times faster, as `Vec3Int.__new__` gained the fast path that `Vec3Float` already had. The other ways of constructing one are unchanged. [#1491](https://github.com/scalableminds/webknossos-libs/pull/1491)
