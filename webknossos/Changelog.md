...
### Fixed
- `Layer.export.as_ozx` wrote misaligned data to the coarser mags when the bounding box's topleft wasn't a multiple of them. The exported region is now enlarged to align with the coarsest exported mag, so all mags line up with the source layer. [#1551](https://github.com/scalableminds/webknossos-libs/pull/1551)
...
- Added `View.data_format`. [#1492](https://github.com/scalableminds/webknossos-libs/pull/1492)
...
- `Dataset.from_images`/`add_layer_from_images` now store the value range observed while reading the images as the color layer's default view configuration `min`/`max`. Values provided by the source format itself (e.g. an OME-Zarr `omero` window) take precedence. [#1525](https://github.com/scalableminds/webknossos-libs/pull/1525)
...
### Fixed
- `RemoteDataset.reopen` now keeps the `read_only` flag of the original dataset. [#1492](https://github.com/scalableminds/webknossos-libs/pull/1492)
