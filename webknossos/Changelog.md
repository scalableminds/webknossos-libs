...
### Changed
- Sped up the `median` and `mode` downsampling filters by computing them with numba kernels that read the source buffer directly, instead of reshaping it into an (elements-per-block, num-blocks) array first. Downsampling a uint8 color layer by a factor of 2 in each dimension is about 6x faster, a uint32 segmentation layer about 3x, and the peak memory per worker drops accordingly. The results are unchanged. Floating point layers keep using the previous implementation. [#1529](https://github.com/scalableminds/webknossos-libs/pull/1529)
