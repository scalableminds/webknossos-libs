...
- `Dataset.from_images`/`add_layer_from_images` now derive a color layer's default view configuration from the data they read: `min`/`max` from the observed value range, and `intensityRange` from an approximate histogram. Values provided by the source format itself (e.g. an OME-Zarr `omero` window) take precedence. [#1525](https://github.com/scalableminds/webknossos-libs/pull/1525)
