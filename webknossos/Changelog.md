...
- Reading a mag via a non-default access mode (e.g. `layer.get_mag(mag, access_mode=RemoteAccessMode.PROXY_PATH)` on a dataset opened with `ZARR_STREAMING`) now makes one additional request the first time each such mode is used per dataset, to fetch and cache that mode's own datasource-properties.json. This replaced computing paths from a fixed url formula, which could not account for a datastore's per-dataset served representation (see Fixed).
