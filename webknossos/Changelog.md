...
### Added
- The access mode of a remote dataset can now be chosen per mag: `RemoteLayer.get_mag(mag, access_mode=RemoteAccessMode.PROXY_PATH)`. Mags of the same layer may use different access modes. Only `DIRECT_PATH` paths are stored in the dataset properties; for any other access mode than the dataset's own default, its bounding box, axis order, `data_format` and mag paths are looked up from that mode's own served datasource-properties.json (or the api data source, for `DIRECT_PATH`) on demand, since the datastore's served representation for a mode can legitimately differ from another mode's (e.g. splitting a multi-channel source into single-channel layers).
- Added `RemoteMagView` with `paths` (a `Mapping[RemoteAccessMode, UPath]` of every available path for that mag), `access_mode`, `data_format` and `with_access_mode()`.
- Added `RemoteDataset.access_mode`, the default access mode that all mags of a dataset inherit.
- Added `RemoteAttachments.with_access_mode()` and `RemoteAttachments.access_mode`. Attachments can be resolved via `DIRECT_PATH` or `PROXY_PATH`; `ZARR_STREAMING` never lists any attachments, since they are not part of its served datasource-properties.json.
- Added `View.data_format`.
- Added support for converting plain Zarr arrays (v2 and v3), OME-Zarr multiscale groups (NGFF 0.4 and 0.5), zipped OME-Zarr archives (`.ozx`, [NGFF RFC-9](https://ngff.openmicroscopy.org/rfc/9/)), plain N5 datasets and N5 multiscale pyramids, and Neuroglancer precomputed volumes via `Dataset.from_images`/`add_layer_from_images`. [#1495](https://github.com/scalableminds/webknossos-libs/pull/1495)
- Added `Layer.export`, providing `as_ozx` (a zipped OME-Zarr archive compliant with [NGFF RFC-9](https://ngff.openmicroscopy.org/rfc/9/)), `as_tiff_stack` (a directory of per-slice TIFF files, one per z-section and per combination of any additional axes) and `as_ome_tiff` (a single pyramidal OME-TIFF file) to export a layer's data, optionally restricted to a bounding box and/or magnification. All three support layers with additional axes (e.g. time); `as_ome_tiff` additionally requires the layer's axes to be a subset of c, t, z, y, x. [#1502](https://github.com/scalableminds/webknossos-libs/pull/1502)
...
### Changed
- `RemoteDataset`'s properties -- which drive layer/mag/attachment management and metadata writes (`add_layer`, `delete_mag`, `default_view_configuration = ...`, etc.) -- are now always sourced from the WEBKNOSSOS api data source whenever one is available, independently of `access_mode`: adding/deleting layers, mags and attachments, and other metadata writes, now work under any default access mode (previously only under `DIRECT_PATH`), except for an annotation's volume layers or a dataset with an unusable data source, which have no api-sourced properties to write back to. `access_mode` (on `RemoteDataset.open`) now only selects the default access mode that a layer's own mags/attachments use for reading image data; it no longer affects what is writable. Consequently, `RemoteLayer.data_format` reports the format of the underlying storage (e.g. `zarr3`) again, not `ZARR_STREAMING`'s re-served `zarr`.
- A mag's `DIRECT_PATH` is now discoverable via `RemoteMagView.paths`/`RemoteLayer.get_mag(mag, access_mode=RemoteAccessMode.DIRECT_PATH)` regardless of the dataset's default access mode, by looking it up from the WEBKNOSSOS api on demand. `RemoteSegmentationLayer.attachments` is likewise now populated regardless of access mode, where it used to be empty outside of `DIRECT_PATH`.
...
- Resolving a mag's path/bounding box/axis order/`data_format` for a given access mode (including the dataset's own default, unless it is `DIRECT_PATH`) now makes one additional request the first time each access mode is used per dataset, to fetch and cache that mode's own datasource-properties.json. This replaced computing paths from a fixed url formula, which could not account for a datastore's per-dataset served representation (see Fixed).
...
- Image files matched by a glob pattern or found in a directory are now ordered with `natsort`, consistently with how `Dataset.from_images` already sorted them. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
...
- Reading a remote mag whose access mode differed from the dataset's own default (or, before per-mag access modes existed, reading through `ZARR_STREAMING`/`PROXY_PATH` at all) could silently return different or all-zero data for a dataset whose WEBKNOSSOS api metadata disagrees with what that access mode's endpoint actually serves — for example an externally explored dataset whose layer is one channel split out of a multi-channel source, where the datastore's own served representation for a given access mode can legitimately use a different channel index, axis order, bounding box or data format than another access mode's document describes. Every mag/layer property is now always sourced from the document actually served by the access mode being read, never mixed across access modes, so `ZARR_STREAMING`, `PROXY_PATH` and `DIRECT_PATH` reads of the same mag now always return identical data.
- After a metadata write (e.g. adding a mag), a mag resolved via an access mode other than `DIRECT_PATH` could raise a spurious "not available for this dataset" error, or otherwise miss the write, because that access mode's own datasource-properties.json had been cached before the write and was never invalidated by it.
- `Dataset.from_images` no longer raises `UnboundLocalError: cannot access local variable 'input_files'` when it is given a single file whose format no reader supports; it now reports the format as unsupported like it does for a directory. [#1496](https://github.com/scalableminds/webknossos-libs/pull/1496)
- Converting a corrupt MRC file no longer produces an empty layer or fails much later with an unrelated error: `mrcfile` parses an unreadable header permissively and reports an empty extent, which is now detected up front. [#1496](https://github.com/scalableminds/webknossos-libs/pull/1496)
...
- Fixed `.ims` and MRC conversion with `mag` greater than 1: it crashed with "zero-size array to reduction operation maximum" once the image spanned more than one shard, and wrote corrupted data when converting compressed layers in parallel. [#1477](https://github.com/scalableminds/webknossos-libs/pull/1477)
...
- `Dataset.add_layer_from_images` now converts a directory of RGB images into a single RGB layer, instead of raising `UnsupportedImageDataError` about their three channels. Whether channels counted as RGB was decided from the suffix of the path it was given, and a directory has none. The same images passed as a glob pattern or as a list of files already converted, as did `Dataset.from_images`, which passes lists. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
- Converting an RGB image (`.png`, `.jpg`, …) no longer emits a spurious "Some images are larger than expected" warning. The expected bounding box reported the channel count as the x extent, so it never matched what was written. The written data and the layer's final bounding box were always correct. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
- Fixed the axis order guessed for very small images: a grayscale image three pixels wide was read as a one-pixel RGB image, and a single-row RGB image as three channels of a one-pixel image. The guess could not tell a channel axis from a real extent of length 1 or 3; every reader now reports its own axes instead. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
- Fixed the order of the extra axes of an image that has two or more beyond x/y/z and channel (e.g. both a time and a scene axis) depending on Python's string hash randomization, which could order the converted slices differently from one run to the next. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
