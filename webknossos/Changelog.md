...
- Added `webknossos.UnsupportedImageFormatError`, raised by `Dataset.from_images` and `Dataset.add_layer_from_images` when no reader can convert the given images, so that callers can catch this case instead of matching on an error message. It subclasses `ValueError` (which both methods raised before) and carries the offending `path` and `suffix`, the `supported_suffixes`, and `missing_extras` — the extras to install when the format is only unsupported because an optional dependency is missing, e.g. `.ims` without `webknossos[ims]`. [#1496](https://github.com/scalableminds/webknossos-libs/pull/1496)
