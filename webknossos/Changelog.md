...
- `Dataset.from_images`/`add_layer_from_images` now store the value range observed while reading the images as the color layer's default view configuration `min`/`max`, so WEBKNOSSOS no longer has to assume the full dtype range. Values provided by the source format itself (e.g. an OME-Zarr `omero` window) take precedence. [#1525](https://github.com/scalableminds/webknossos-libs/pull/1525)
