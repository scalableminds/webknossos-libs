...
### Fixed
- `Layer.export.as_ozx` wrote misaligned data to the coarser mags when the bounding box's topleft wasn't a multiple of them. The exported region's topleft is now floored to the coarsest exported mag, so all mags line up with the source layer. [#1551](https://github.com/scalableminds/webknossos-libs/pull/1551)
