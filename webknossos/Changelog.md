...
- Added `Vec3Float` and `Vec3FloatLike` to `webknossos.geometry`, the float counterpart to `Vec3Int` / `Vec3IntLike`. `Vec3Float` mirrors `Vec3Int`: it offers `x`/`y`/`z`, element-wise arithmetic, and `to_tuple`/`to_list`/`to_np`/`to_vec3_int`, and it compares and hashes equal to the corresponding plain tuple. Float triples such as a voxel size or a node rotation now use it instead of the `tuple[float, float, float]` aliases that seven modules each declared for themselves. [#1491](https://github.com/scalableminds/webknossos-libs/pull/1491)
