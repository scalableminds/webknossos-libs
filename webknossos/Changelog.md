...
- Replaced the `scm-pims` dependency with an in-library implementation of the parts this library used, and dropped `scm-pims` and `slicerator`. Two consequences: formats that were only reachable through readers `pims` bundled can no longer be converted — `.jp2`/`.j2k`, `.webp`, `.ppm`/`.pgm`/`.pbm`, `.tga`, `.sgi`, `.exr`, `.hdr`, `.psd`, `.dds`, `.pcx`, `.ras`, `.xbm`/`.xpm`, `.lsm`, `.stk`, `.cine`, `.seq` and `.spe` — and `Dataset.add_layer_from_images` no longer accepts a `pims.FramesSequence` instance, only paths, glob strings and sequences of paths. TIFF, CZI, DM3/DM4, `.png`/`.jpg`/`.jpeg`/`.gif`/`.bmp`/`.ico`, `.ims` and MRC are unaffected. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
...
### Changed
- `.czi` files are now read as a chunk-based format: each chunk fetches only the box it needs instead of decoding whole planes, the layer's bounding box is exact from the start rather than a placeholder corrected afterwards, and multi-timepoint files convert into a single layer with a `t` axis. A `.czi` carrying a rotation, illumination, phase, view or block dimension longer than 1 is now refused with `UnsupportedImageDataError` instead of being written to an extra axis; scenes are unaffected. Missing `pylibCZIrw` now names the `webknossos[czi]` extra instead of silently dropping `.czi` from the supported formats. [#1498](https://github.com/scalableminds/webknossos-libs/pull/1498)
