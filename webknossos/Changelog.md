...
### Fixed
- Sped up `Dataset.from_images`/`add_layer_from_images` for the chunk-based formats (Zarr, OME-Zarr, `.ozx`, N5, Neuroglancer precomputed, `.ims`, MRC, `.czi`) when writing to a non-WKW layer. Every chunk was materialized into Fortran order before being written, which for a large shard is a full cache-hostile transposing copy of the whole chunk — measurably the most expensive step of the conversion. Only WKW needs that layout; the other formats are written through TensorStore, which handles arbitrary strides itself. [#1528](https://github.com/scalableminds/webknossos-libs/pull/1528)
