...
- Reading through `ZARR_STREAMING`/`PROXY_PATH` for a dataset whose WEBKNOSSOS api metadata disagrees with what those endpoints actually serve (e.g. some externally explored datasets) could silently return all-zero data, because the bounding box/axis order used for the read came from the api instead of the endpoint being read. `self._properties` is now always sourced from the access mode actually being read, and metadata writes require `DIRECT_PATH` again for the same reason.
