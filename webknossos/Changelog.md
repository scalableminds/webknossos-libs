...
### Added
- Added support for converting plain Zarr arrays (v2 and v3), OME-Zarr multiscale groups (NGFF 0.4 and 0.5), plain N5 datasets and N5 multiscale pyramids, and Neuroglancer precomputed volumes via `Dataset.from_images`/`add_layer_from_images`. For a multiscale source, the finest resolution level is converted as `Mag(1)`; the rest of the mag pyramid is built by the usual `downsample=True`/`Layer.downsample()` afterwards, as with every other format. Unlike `.ims`, MRC and CZI, these formats also support remote input paths (`s3://`, `gs://`, `http(s)://`). Builds on the `ChunkedImageSource` abstraction introduced for `.ims` and MRC. [#1495](https://github.com/scalableminds/webknossos-libs/pull/1495)
